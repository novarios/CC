...
int main(int argc, char * argv[])
{
  ...
  if(Parameters.basis != "finite_J" || test != 1){
    //Perform_CC(Parameters, Space, Chan, Ints, Amps, HF_Chan, HF_ME);
    ...
    if(Parameters.approx == "singles"){ std::cout << "Eref = " << Energy << ", dCCSD = " << Energy0 << std::endl; }
    else{ std::cout << "Eref = " << Energy << ", dCCD = " << Energy0 << std::endl; }      
    Energy += Energy0;
    std::cout << "E = " << Energy << ", E/A = " << Energy/(Parameters.P + Parameters.N) << std::endl;
  }

  if(Parameters.extra == -1 || Parameters.extra == 1){
    State *states = new State[3];
    double *nums = new double[6];
    CC_Eff V_Eff(Parameters, Space, Chan);
    Build_CC_Eff(Parameters, Space, Chan, Ints, Amps, V_Eff);
    if(Parameters.extra == 1){ PA_EOM(Parameters, Space, Chan, V_Eff, states, nums); }
    else if(Parameters.extra == -1){ PR_EOM(Parameters, Space, Chan, V_Eff, states, nums); }
    for(int i = 0; i < 3; ++i){
      std::cout << Parameters.Shells << "\t" << Parameters.Pshells << "\t" << states[i].ml << "\t" << states[i].m << "\t"
		<< Parameters.density << "\t" << Energy << "\t" << nums[2*i] << "\t" << Energy + nums[2*i] << "\t" << nums[2*i + 1] << std::endl;
    }
